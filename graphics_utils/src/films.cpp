...
float cookTorranceFilms(complex etaI, complex etaF, complex etaT, float thickness, float lambda, float roughness,
                        float3  normal, float3 viewDir, float3 lightDir) 
{
    ...
    float fresnel = FrFilmRefl(cosThetaH, etaI, etaF, etaT, thickness, lambda);
    ...
    float specularReflection = (fresnel * ndf * g) / (4.0f * std::max(LiteMath::dot(normal, viewDir), 0.0f) * std::max(LiteMath::dot(normal, lightDir), 0.0f) + EPSILON);
...
py::array_t<float> film_brdf(py::array_t<float> w_i_numpy, py::array_t<float> w_o_numpy, py::array_t<float> w_n_numpy, py::array_t<float> etaI_numpy, py::array_t<float> etaF_numpy, py::array_t<float>  etaT_numpy, float thickness, float alpha, py::array_t<float> lambdas) 
{
  ...
  for(int i = 0; i < lambdas_info.size; ++i)
    resultT[i] = cookTorranceFilms(*etaI, *etaF, *etaT, thickness, lambdasT[i], alpha, *w_n, *w_i, *w_o);
