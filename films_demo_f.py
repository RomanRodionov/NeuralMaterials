...
from graphics_utils import film_refl, real_fourier_moments
from dataset.materials import FourierIridescenceDataset
...
def train_decoder():
    ...
    dataset = FourierIridescenceDataset(max_wavelength=WL_MAX, min_wavelength=WL_MIN, n_samples=samples, wl_samples=wl_samples, moments=MOMENTS)
    ...
    decoder = FSpectralDecoder(hidden_dim=10, output_dim=MOMENTS, wavelength_max=WL_MAX, wavelength_min=WL_MIN).to(device)
    ...
    for batch, data in enumerate(progress_bar):
        ...
        if batch % 25 == 0:
            progress_bar.set_description(f"Batch {batch}, MSE loss: {loss1.item():.6f}, {loss2.item():.6f}, SAM: {sam.item():.4f}")
...
if __name__ == "__main__":
    ...
    w_o = np.array((0.5, 0.0, 0.5))  # doesn't mean yet
    ...
    for i, w_i in enumerate(w_i_vectors):
        ...
        gt_map[i] = film_refl(w_i, w_o, eta_i, eta_f, eta_t, thickness, wavelengths)
